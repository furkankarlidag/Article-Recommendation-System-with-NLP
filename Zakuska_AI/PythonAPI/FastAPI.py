...
def search_for_articles_by_text(user_id: str, searchKey: str):
    try:
        ...
        search_vector = get_fasttext_vector(processed_text)
        
       
        interest_vectors = {}
        if 'ilgiAlani' in user_document:
            for interest in user_document['ilgiAlani']:
                processed_interest = preprocess_text(interest)
                interest_vector = get_fasttext_vector(processed_interest)
                interest_vectors[interest] = interest_vector

        
        combined_vector = search_vector
        if interest_vectors:
            combined_vector = np.mean([search_vector] + list(interest_vectors.values()), axis=0)

       
        recommendations = recommend_articles_based_on_interests(combined_vector, article_vectors)
...
def get_scibert_advanced_recommendations(user_id: str, searchKey: str):
    ...
    for text in user_document.get("ilgiAlani", []) + [searchKey]:
